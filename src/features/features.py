...
def length_Betti(persistence_diagram, n=2, homology_dim=0):
    ...
    return \
        np.partition(np.diff(persistence_diagram[:, :2][persistence_diagram[:, -1] == homology_dim]).flatten(), -1 * n)[
            -1 * n]
...
def onset_longest_Betti(persistence_diagram, homology_dim=1):
    ...
    return persistence_diagram[:, :2][persistence_diagram[:, -1] == homology_dim][
        np.diff(persistence_diagram[:, :2][persistence_diagram[:, -1] == homology_dim]).argmax()][0]
...
def polynomial_feature_2(persistence_diagram, homology_dim):
    ...
    if homology_dim in persistence_diagram[:, -1]:
        N = persistence_diagram[persistence_diagram[:, -1] == homology_dim].shape[0]
        return ((persistence_diagram[persistence_diagram[:, -1] == homology_dim][:, 1].max() - persistence_diagram[
                                                                                                   persistence_diagram[
                                                                                                   :,
                                                                                                   -1] == homology_dim][
                                                                                               :, 1])
                * (persistence_diagram[persistence_diagram[:, -1] == homology_dim][:, 1] - persistence_diagram[
                                                                                               persistence_diagram[:,
                                                                                               -1] == homology_dim][:,
                                                                                           0])).sum() / N
    else:
...
def computing_distance_matrix(graph):
    ...
    l_new = np.array(list(
        list(i) for i in product(list(range(len(nodes))), list(range(len(nodes)))) if tuple(reversed(i)) >= tuple(i)))
...
def create_non_TDA_features(directory):
    ...
    for f in cat_feats:
        lbl = LabelEncoder()
        lbl.fit(train[f].to_list() + test[f].to_list())
        train[f] = lbl.transform(train[f].to_list())
        test[f] = lbl.transform(test[f].to_list())
    ...
    print('Score is:', metrics(pd.concat([X_val, y_val], axis=1), preds))
